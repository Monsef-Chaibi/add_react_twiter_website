...
<body class="bg-dark">
    <header>
        ...
        $sql = "SELECT * FROM contact where user_from = 14 or user_to = 14 ORDER BY id asc"; // Change to your table name
        ...
        if (isset($_POST["submit"])) { 
            ...
            try {
                ...
                $stmt = $connect->prepare("INSERT INTO contact (name,user_from, user_to ,msg) VALUES (:name,:user_from,:user_to,:msg)");
                $stmt->bindParam(':name', $_SESSION["user"]);
                $stmt->bindValue(':user_from', 'admin', PDO::PARAM_STR);
    ...
    <span class="text-center text-light mt-4 contact100-form-title">
        ...
        <?php
            foreach($result as $contact): 
                    ?>
                    <div class="aff">
                        <input disabled style="border:2px solid black; margin:10px;border-radius: 30px;background-color:black<?php if($contact['user_from']==='admin'){echo 'color:black;background-color:white';} ;?>" type="text" value=" <?php echo $contact['msg']; ?>"><br>
                    </div>
                     <?php endforeach; ?>
